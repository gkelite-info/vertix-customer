...
export default function TableBody({
  ...
  return (
    <tbody>
      {data.map((row, index) => {
        const isTotalRow = row.taxType === "Total";
        return (
          <tr
          ...
          >
            {columnKeys.map((key) => {
              ...
              const isTotalNegative = isTotalRow && (key === "beforePlanning" || key === "afterPlanning") &&
                typeof value === "number" && value < 0;

              return (
                <td key={key} style={{ ...tdStyle, color: isTotalNegative ? "#DC2626" : "#1D2B48", fontWeight: isTotalRow ? "600" : "normal", }} title={String(value)}>
                  {String(value)}
                </td>
              );
            })}
