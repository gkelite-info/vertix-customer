...
export default function TaxDashboard() {
    const [activeTab, setActiveTab] = useState<'Getting to know you' | 'About You' | 'Dependents' | 'Residency Details' | 'Income Details' | 'Deduction Details' | 'FBAR/FATCA'>('Getting to know you');
        ...
        <div className="bg-white lg:h-auto pb-7">
            ...
            <div className="flex justify-center gap-5 mt-3">
                <div className="bg-[#FFFEFE] w-[28%] h-102 p-2 pt-4 pb-4 rounded-lg text-center flex flex-col items-center shadow-md">
                    <h3 className="text-[#1D2B48] font-semibold mb-4">Tax Organizer Dashboard</h3>
                    {['Getting to know you', 'About You', 'Dependents', 'Residency Details', 'Income Details', 'Deduction Details', 'FBAR/FATCA'].map(tab => (
                        <button
                            key={tab}
                            onClick={() => setActiveTab(tab as any)}
                            className={`block w-[90%] text-left px-3 py-2 rounded-md mb-2 cursor-pointer
                ${activeTab === tab ? 'bg-[#4F46E5] text-white font-medium' : 'bg-[#E8E8E8] text-[#1D2B48] font-medium hover:bg-blue-500/30'}`}
                        >
                            {tab}
                        </button>
                    ))}
